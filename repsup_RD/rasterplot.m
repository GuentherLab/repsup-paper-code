...
load('/projectnb/busplab/Experiments/ECoG_Preprocessed_RD/LocalEpoched/S376/Epoch_stimuli_12_Hilbert_HG.mat')
...
%% params
electrode_ind = [19 32 38 40 41 46 51];
electrode_ind = setdiff(1:71, electrode_ind);
...
%         imagesc([1 size(trials1.dat(rowmatch,:),2)], [1 size(trials1.dat(rowmatch,:),1)],...
%             trials1.dat(rowmatch, :));
        ...
        imagesc([1 size(trials2.dat(rowmatch,:),2)], [1 size(trials2.dat(rowmatch,:),1)],...
            trials2.dat(rowmatch, :));
        caxis([minval, maxval]); % use same color scale for all plots
        cbar = colorbar; 
        if icon == 1; title({'word 2',thiscon}); else title(thiscon); end
        if icon == ncons; xlabel('Time (ms)'); end
        ylabel('Trial Index')
        if show_1sec_line
            hline_stim = line([line_xloc_stim, line_xloc_stim], ylim,'Color',line_color_1sec,'LineWidth',line_width,'LineStyle',line_style);
